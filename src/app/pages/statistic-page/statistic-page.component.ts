...
export class StatisticPageComponent implements OnInit {
	...
	marketPriceChart = {
		...
		options: {colors: ['#f2a900', '#f3b49f', '#f6c7b6']},
	}
	marketData!: any
	...
	async ngOnInit(): Promise<void> {
		...
		if (!this.tranData) {
			...
			this.tranData = (await this.bitcoinService.getConfirmedTransactions()).subscribe(
				(res: any) => {
					this.tranData = res.values.map((value: {x: any; y: any}) => [value.x, value.y])
		...
		if (!this.marketData) {
			...
			this.marketData = (await this.bitcoinService.getMarketPrice()).subscribe((res: any) => {
				this.marketData = res.values.map((value: {x: any; y: any}) => [
					new Date(value.x),
					+value.y,
				])
